...
```yaml
# --- 模型配置 ---
model_name: facebook/dinov2-base     # 默认使用 base；可根据显存选择 small/base/large
...
# --- 训练参数 ---
epochs: 20
batch_size: 1024                      # 较大的 batch size 有助于对比学习
...
训练过程中，模型权重和日志将保存在 `experiments/output_sop_dinov2_i2i` 目录下。
...
```bash
python run.py examples/dinov2_i2i.yaml --config_override lr=0 model_name_or_path=experiments/output_sop_dinov2_i2i/checkpoint-1000
```
...
**训练配置：**
- 模型：facebook/dinov2-base（默认）
- 数据集：SOP 训练集（~120k 张图片）
- 训练轮次：2 epochs
- Batch Size：128
- LoRA：启用

微调后的 DINOv2-base 在 Recall@1 上相比零样本预训练模型获得了 **24.94%** 的显著提升，达到 **79.97%** 的高精度，可广泛应用于生产环境中的商品检索系统。
...
方式一：编辑 YAML 配置文件
```bash
# 修改 examples/dinov2_i2i.yaml 中的参数，然后运行
python run.py examples/dinov2_i2i.yaml
```
...
| 模型 | 参数量 | 显存占用 | 推荐场景 |
| :--- | :--- | :--- | :--- |
| dinov2-small | ~21M | 低 | 边缘设备、快速迭代 |
| **dinov2-base**（默认） | ~86M | 中等 | **平衡性能与速度** ✓ |
| dinov2-large | ~300M | 高 | 追求最高精度 |

根据 README 实验结果，**dinov2-base** 在 SOP 数据集上微调后能达到 79.97% 的 Recall@1，是生产环境的最佳选择。
...
训练后会生成：
```
experiments/output_sop_dinov2_i2i/
