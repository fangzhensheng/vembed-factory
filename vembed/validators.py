...
class ConfigValidator:
    ...
    def _validate_data(self) -> None:
        ...
        if not data_path:
            self.errors.append(
                "❌ data_path is required (path to training JSONL file). "
        ...
        if retrieval_mode in ("t2i", "i2i", "m2i"):
            ...
            if not image_root:
                self.warnings.append(
                    f"⚠️  retrieval_mode='{retrieval_mode}' usually needs --image_root"
                )
            elif not os.path.exists(image_root):
                self.warnings.append(
                    f"⚠️  image_root directory not found: {image_root}"
                )
    ...
    def _validate_loss_pooling(self) -> None:
        ...
        if loss_type == "colbert":
            if pooling_method and pooling_method != "none":
                self.warnings.append(
                    f"⚠️  loss_type='colbert' works best with pooling_method='none' "
                    f"(not '{pooling_method}'). Auto-correcting."
                )
        else:
            if pooling_method == "none":
                self.warnings.append(
                    f"⚠️  loss_type='{loss_type}' requires global pooling "
    ...
    def _validate_distributed(self) -> None:
        ...
        if use_fsdp and use_gradient_cache:
            self.warnings.append(
                "ℹ️  Using both FSDP and Gradient Cache. This is fine for very large models."
            )
        ...
        if use_fsdp:
            ...
            if not num_gpus:
                self.warnings.append(
                    "⚠️  Using FSDP but num_gpus not specified. "
                    "Set with --num_gpus <N>"
                )
    ...
    def _validate_distillation(self) -> None:
        ...
        if teacher_model:
            ...
            if model_name and model_name == teacher_model:
                self.warnings.append(
                    "⚠️  Student model same as teacher model. "
