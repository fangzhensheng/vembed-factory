...
class SimpleContrastiveLoss:
    ...
    def __call__(self, x: Tensor, y: Tensor, target: Tensor = None, reduction: str = "mean"):
        if target is None:
            ...
            target = torch.arange(
                0, x.size(0) * self.target_per_qry, self.target_per_qry, device=x.device
            )
...
class ContrastiveLossWithQueryClosure(SimpleContrastiveLoss):
    def __call__(
        ...
        *reps: Tensor,
        ...
        if len(reps) == 2:
            assert (
                query_closure is None
            ), "received 2 representation tensors while query_closure is also set"
