...
class SimpleLitModel(pl.LightningModule):
    ...
    def init_gc(self, scaler, ddp_module):
        ...
        print(
            f"*** initializing gradcache with ddp_module={type(ddp_module)}, minibatch_size={self.params.gc_minibatch_size}"
        )
        self.gc = PLGradCache(
            models=[ddp_module],
            ...
            fp16=self.fp16,
            scaler=(
                scaler if self.fp16 else None
            ),  # needed when using automatic_optimization is off and fp16 is on
...
def get_argument_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default="nccl",
        help="torch distributed backend (Default: nccl), use 'gloo' if nccl doesn't work",
    )
    ...
    # grad cache params
    parser.add_argument(
        "--use_gc", action="store_true", default=False, help="whether to use grad cache"
    )
    parser.add_argument(
        "--gc_minibatch_size",
        type=int,
        default=2,
        help="mini batch size of grad cache, must be provided if use_gc is on",
    )
